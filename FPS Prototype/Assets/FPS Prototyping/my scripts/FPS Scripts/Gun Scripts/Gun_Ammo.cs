...
namespace S3
{
    public class Gun_Ammo : MonoBehaviour
    {
        ...
        public void OnReloadComplete()
        {
            for(int i = 0; i < ammoBox.typesOfAmmunition.Count; i++)
            {
                if(ammoBox.typesOfAmmunition[i].ammoName == ammoName)
                {
                    ...
                    if(ammoBox.typesOfAmmunition[i].ammoCurrentCarried < ammoTopUp && ammoBox.typesOfAmmunition[i].ammoCurrentCarried != 0)
                    {
        ...
        {
            yield return new WaitForSeconds(0.05f); //This is a fudge fator to ensure that the UI is updated when changing weapons
            UIAmmoUpdateRequest();
        }
